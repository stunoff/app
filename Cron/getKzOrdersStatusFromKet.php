        ...
        foreach ($result as $ketOrderId => $ketData) {
            ...
            if ($ketData['kz_delivery'] == 'Почта') {
                if (array_key_exists($ketSendStatus, KetModel::$finalPostStatusMap)) {
                    ...
                    // Ждем финальных статусов (оплачен/возврат) по таким заказам
                    if ($ordersOldInfo[$ketOrderId]['status'] == 0
                        && !empty($ordersOldInfo[$ketOrderId]['post_number'])) {
                        $ketRepo->writeLog($orderId, $ketStatusKz, $ketSendStatus, $orderStatus, $orderStatusLs, 'Ждем финальных статусов. Статусы заказа НЕ обновлены');
                        continue;
                    }
